...
- `.claude/` — this repo's own Claude Code config (hooks, slash commands, project subagents). See below.
- `.agents/` — multi-agent collaboration protocol shared with other autonomous agents working in this workspace. See below.
...
## This repo's own `.claude/` configuration

This repository is itself set up as a Claude Code project, separately from the skills it contains:

- `.claude/settings.json` — a `SessionStart` hook plus a permissions allowlist for common read-only Bash commands (`git status`, `ls`, `cat`, `find`, etc.) and skill installs.
- `.claude/hooks/session-start.sh` — on Claude Code **web/remote sessions only** (guarded by `CLAUDE_CODE_REMOTE`), auto-installs every skill's `requirements.txt` via `pip3 install --user`. No-op locally; you manage your own env there.
- `.claude/commands/` — two project slash commands: `/new-skill` (scaffolds a `skills/<name>` directory from `template/SKILL.md` and registers it in `marketplace.json`) and `/validate-skill` (checks a skill's frontmatter, naming, and marketplace registration). These overlap with `skills/skill-creator`'s fuller workflow — use the slash commands for quick scaffolding/checks, `skill-creator` for the full authoring/eval loop.
- `.claude/agents/` — project subagents available on every Claude Code surface for this repo: `skill-reviewer.md` (first-party, reviews skill quality — see `skill-reviewer` in the skill list) plus 243 files vendored as-is from [msitarzewski/agency-agents](https://github.com/msitarzewski/agency-agents) (MIT-licensed, see `AGENCY-AGENTS-LICENSE.txt`). The vendored agents have human-readable `name` values (e.g. "Incident Response Commander"), not slugs — invoke by describing the task/role, not a hyphenated ID. They are unrelated to this repo's actual content (skills authoring); don't assume they know repo conventions.
...
Companion files in `.agents/`:
- `TOPOLOGY.md` — who the agents are, their branch-prefix/identity/role, which Claude Code surfaces (web/phone vs. CLI/VS Code) share the `claude/*` namespace, and the roster of repos in the wider collaboration workspace (this repo is the canonical source for `.agents/`, propagated to the others).
- `HERMES_ONBOARDING.md` — setup commands (labels, branch protection) Hermes runs when onboarding a new repo into the workspace; not something Claude Code needs to execute itself.
- `SETUP_REPORT.md` — a point-in-time status report (generated by Hermes) of which repos in the workspace have the collaboration protocol fully wired up; informational, not canonical — treat `COLLABORATION.md`/`TOPOLOGY.md` as the source of truth if they disagree.
